...
def test_variant_b_contract_is_backed_by_real_devctl_and_mcp_mappings() -> None:
    ...
    assert model["mappings"]["web.test.e2e"] == ["./dev/bin/govctl run web.test.e2e --json"]
    assert model["mappings"]["web.test.integration"] == ["./dev/bin/govctl run web.test.integration --json"]
