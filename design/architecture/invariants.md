...
| INV-S05 | The permanent library (`/nightfall/media/pictures`) is read-only to the ingress service — no write path crosses this boundary | all modules | No write call site targets the permanent library path; `hash-import` may recompute stale/missing/invalid cache data ephemerally but never writes to the permanent library | `design/domain-architecture-overview.md` §1, §3 |
...
| INV-HI10 | Imported hashes MUST NOT infer source-account ownership from path/metadata | `hash_import` module | Hash-import rows use fixed internal account scope (`account_name = '__hash_import__'`), with `source_relpath = NULL`; no source-account mapping is derived | Issue #65 invariant 10 |
