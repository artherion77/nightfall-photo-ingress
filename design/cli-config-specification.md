...
| `--stop-on-error` | off | Abort on first unrecoverable error |
...
3. Verify that each subsequent line:
  - contains exactly 3 tab-separated fields
...
10. Imported hashes MUST NOT infer source-account ownership. Hash-import rows use a fixed internal account scope (`__hash_import__`) and MUST NOT derive account identity from path or metadata.
...
A successful `hash-import` creates or confirms an `external_hash_cache` row with
...
Imported entries are used solely for dedupe index lookups during ingest
pre-download filtering. They do not participate in the `files.status` state
machine and carry no lifecycle semantics.
...
| `sync_hash_import_path` | path | none | Pass `<path-to-root>` as argument to `hash-import`. **Parser-required in the current runtime for legacy sync-import compatibility — see note below.** |
...
## 8. [import] Section (Issue #65)

The `[import]` section provides defaults for the `hash-import` command.

> **Implementation status:** Implemented. The config parser accepts `[import]` and
> applies defaults when the section is absent.
...
- Unsupported section names (anything other than `core`, `logging`, `web`, `import`, `account.*`) cause a validation error.
