...
Phase 1 targets a fully functional localhost-only control plane with core security
guardrails in place. LAN exposure, TLS, and reverse proxy introduction remain
...
### 2.1 Recommended execution sequence from the current state

With Chunks 0-5 complete, execute remaining work in this order:

1. Finish Chunk 6 (localhost hardening completion).
2. Run a stability pass on all existing API + UI integration suites.
3. Open Phase 2 mandatory LAN-exposure work (reverse proxy, TLS, proxy-level security
  controls, deployment/rollback mechanics) only after Chunk 6 sign-off.

This sequencing preserves minimal-change risk: close local hardening first, then move
to network-exposed topology changes.
...
- Auth or security configuration.
...
- `design/web/roadmaps/web-control-plane-integration-plan.md` §4 (endpoints, auth, migrations)
...
- Request throttling (deferred to Phase 2) or CORS headers.
...
- `design/web/roadmaps/web-control-plane-phase1-scope.md` §3.3 (C3: health store lifecycle), §3.11 (C11: KPI thresholds from API), §3.12 (C12: blur tokens)
...
- `design/web/roadmaps/web-control-plane-integration-plan.md` §5 (phase 2 data-flow)
- `design/web/roadmaps/web-control-plane-phase1-scope.md` §3.10 (C10: LoadMoreButton), §3.11 (C11: KPI thresholds from API config endpoint)
...
- `design/web/roadmaps/web-control-plane-integration-plan.md` §6 (triage design)
...
- Request throttling remains deferred to Phase 2 proxy-level controls.
...
- This chunk now focuses on security posture completion that is meaningful for the
  current localhost deployment and directly supports the later Phase 2 LAN gate.
- Request throttling is fully deferred to the Phase 2 proxy-level LAN gate.
- No in-process request-throttling implementation is required for Phase 1 completion.
...
```
api/app.py             — Extended:
                           explicit CORS policy for localhost and configured origin(s)
                           explicit response-header policy for current deployment mode
...
tests/
  integration/api/test_auth.py       — extended assertions for auth-failure audit side effects
  integration/api/test_*             — CORS/header behavior assertions where applicable
```
...
| A08 Software Integrity | Request throttling deferred to Phase 2 proxy layer; no unsigned redirects |
...
1. Every HTTP 401 and 403 response triggers an audit-log write with client IP, path, and method.
...
- Proxy-level rate limiting implementation (Phase 2 mandatory LAN gate).
...
| Proxy-level rate limiting | Mandatory | Re-Evaluation C2 |
...
### Source code
- `api/` — complete FastAPI application: routers, services, schemas, auth, audit hook, CORS/header posture, auth-failure audit behavior
...
The following drift corrections were applied in this update:

- Rebased status to reflect delivered Chunks 0-5.
- Corrected test-path references to the current `tests/integration/api/` and
