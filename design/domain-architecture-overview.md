...
| Ingest spec (poll cycle, hash import) | [design/specs/ingest.md](specs/ingest.md) |
...
- **Hash import**: `external_hash_cache` stores hashes imported from the permanent library. The `hash-import` CLI command (Issue #65) imports authoritative SHA-256 from `.hashes.v2` files. Legacy `sync-import` imported advisory SHA-1 from `.hashes.sha1` files (deprecated). Imported entries are used for dedupe index lookups during ingest pre-download filtering only. See [architecture/invariants.md](architecture/invariants.md) §Hash Import Invariants.
...
### 6.3 Hash Import (Issue #65)
   ...
   - For the full CLI specification, see [cli-config-specification.md](cli-config-specification.md) §3.
   - Legacy `sync-import` command is deprecated. See [rationale/deprecated-concepts.md](rationale/deprecated-concepts.md).
...
│       ├── sync_import.py       — advisory SHA1 import from permanent library (deprecated; replaced by hash-import)
│       ├── hash_import.py       — authoritative SHA-256 import from .hashes.v2 files (Issue #65)
...
| 7 — Hash Import + Live Photo | `hash-import` CLI (Issue #65), `external_hash_cache`, Live Photo pairing heuristics; `sync-import` deprecated | Delivered |
...
live_photo_component_order = photo_first
live_photo_conflict_policy = nearest_capture_time
sync_hash_import_enabled = true    # DEPRECATED — use hash-import CLI instead
sync_hash_import_path = /nightfall/media/pictures   # DEPRECATED
sync_hash_import_glob = .hashes.sha1   # DEPRECATED
...
11. Run `nightfall-photo-ingress hash-import /nightfall/media/pictures --stats`; confirm imported hashes reduce candidate downloads
...
2. **Hash import model**: The `hash-import` command (Issue #65) imports authoritative SHA-256 hashes from `.hashes.v2` files, eliminating the advisory SHA-1 layer. The legacy `sync-import` command and its `verify_sha256_on_first_download` verification step are deprecated. *(Resolved: DEC-20260412-01.)*
