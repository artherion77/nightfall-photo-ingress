...
| **audit_log** | Append-only SQLite table recording every state transition and pipeline event. Rows are protected against update and delete by DB-layer SQL triggers. | `domain/registry.py` | `actor` is always one of `pipeline`, `cli`, or `trash_watch`. |
...
| **external_hash_cache** | SQLite table storing advisory SHA1 hashes imported from `.hashes.sha1` files in the permanent library. Used as a pre-filter to reduce unnecessary downloads. | `domain/registry.py` | SHA1 entries are advisory only. `verified_sha256` is populated only after a first-download SHA-256 confirmation, after which the entry can gate future metadata-only skips. |
...
| **IngestDecisionEngine** | Domain class that applies the hash-based policy matrix to a `DownloadedHandoffCandidate`. Determines the outcome (`pending`, `rejected_duplicate`, `discard_pending`, etc.) and performs the registry write and file move. | `domain/ingest.py` | Source-agnostic; works identically for any adapter. |
...
| **sync import** | CLI subcommand (`nightfall-photo-ingress sync-import`) that reads `.hashes.sha1` files from the permanent library and seeds `external_hash_cache` with advisory SHA1 hashes to reduce future download volume. | `sync_import.py` | Does not write to the permanent library. Imported SHA1 data is advisory; server-computed SHA-256 remains canonical identity. |
