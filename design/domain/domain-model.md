...
| **UI idempotency replay** | `ui_action_idempotency` | `api/services/triage_service.py` + `domain/registry.py` table bootstrap | Stores triage mutation replay responses keyed by `X-Idempotency-Key`; prevents duplicate state changes for retried requests. |
