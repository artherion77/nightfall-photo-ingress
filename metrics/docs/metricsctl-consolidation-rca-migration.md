...
#### Code changes
- [x] Add `find_repo_root(sentinel="pyproject.toml", max_depth=12) -> Path` to `dev/lib/`
- [x] Add tests for `find_repo_root()` in `tests/unit/` (found, not-found, depth-exceeded, from subdirectory)
- [x] Add `repo_root: Path | None = None` parameter to `venv_bootstrap.ensure_venv()`; maintain backward-compat default
- [x] Update `tests/unit/test_venv_bootstrap.py` with test for explicit `repo_root` parameter
- [x] Update `./metricsctl` `_repo_root()` (line 77) to call `find_repo_root()`
- [x] Update `./metricsctl` `DEV_LIB` computation (line 18) to use `find_repo_root()` result
- [x] Add test that `metricsctl._repo_root()` returns correct path when `CWD` ≠ repo root

#### Caller updates
- [x] Update `govctl-targets.yaml` (`metrics.collect.backend`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.status`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.run-now`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.collect.frontend`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.aggregate`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.generate.dashboard`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.publish`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.install`) to use `./dev/bin/metricsctl`
- [x] Update `govctl-targets.yaml` (`metrics.stop`) to use `./dev/bin/metricsctl`

#### Documentation updates
- [x] Add compatibility shim notice comment to `dev/bin/metrics-runner`

#### Regression gates (must pass before Phase 2 starts)
- [x] All govctl `metrics.full` group targets pass
- [x] `tests/unit/test_mcp_metrics_tasks.py` assertions pass
- [x] `./dev/bin/metricsctl paths` returns correct repo paths when `CWD` ≠ repo root
- [x] `./dev/bin/metrics-runner run/status/publish-github` delegates correctly
...
| Step | Commit | Summary |
|------|--------|---------|
| 1a | `4bfdbb7` | Added `dev/lib/find_repo_root.py` — sentinel-based, anchor-relative, cached discovery |
| 1b | `6257da4` | Added `tests/unit/test_find_repo_root.py` — 7 tests covering found/not-found/depth/cache/real-repo |
| 2  | `45c5a8f` | Added `repo_root` param to `ensure_venv()` + 2 new tests; backward-compatible |
| 3  | `5450cd8` | Updated `metricsctl._repo_root()` and `ensure_venv` call to use `find_repo_root(anchor=Path(__file__))` |
| 4  | `5c91edf` | All 9 govctl metrics targets now use `./dev/bin/metricsctl` directly (removed venv-detection shell blocks) |
| 5a | `310a19a` | Added CWD-independence test for `_repo_root()` |
| 5b | `1f0d23d` | Added compatibility shim notice to `dev/bin/metrics-runner` |
| 6  | `5f4e7b3` | Fixed `ensure_venv` symlink detection — use `is_running_in_venv()` instead of resolved path comparison |

Regression gate results:
- 496 unit tests passed (0 failures)
- `./dev/bin/metricsctl paths` works from `CWD=/tmp`
- `./dev/bin/metrics-runner help` outputs correct usage
- `test_mcp_metrics_tasks.py` assertions pass
